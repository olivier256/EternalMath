...
class MethodAreaPanel extends Panel {
	...
	MethodAreaPanel(int memoryLocationsVisibleCount, int[] theProgram, String[] theMnemonics) {
	...
	// UpdateView() repaints the rightmost three columns: address, byte, and
	// mnemonic
	// starting with the passed initialaddress of theProgram.
	private void updateView(int initialAddress) {

		int filledRowsCount = theProgram.length < memoryLocationsVisibleCount ? theProgram.length
	...
	// the other three columns--address, byte, and mnemonic--by calling
	// updateView().
